...
- Vocab mistake sessions can be created only for the owner's currently active mistake. A wrong retry creates a fresh session instead of changing the accepted answer in an existing one.
