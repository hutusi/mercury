...
- **Per-user daily message cap** — the app's first AI cost control: user-role rows are counted per `(userId, day)` (`day` is computed in `user_settings.time_zone`, the `activity_days` convention); `MERCURY_CHAT_DAILY_LIMIT` (default 30, min 1) → `429 chat_limit_reached`. Two concurrent sends can both pass the count check; accepted here — this is cost control, not billing.
