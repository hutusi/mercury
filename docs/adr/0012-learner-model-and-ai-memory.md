...
- **Signal hooks never fail the parent mutation.** Services fold signals in guarded try/catch after their own writes commit; a profile failure logs and moves on.
...
- Pre-existing users have no row; `ensureLearnerProfile` lazily creates defaults, and the API serializer returns a default shape so clients never branch on null.
