...
- `bun run content:book-audit [<slug>…]` (books only; not part of the verify gate, but **exits non-zero on hard violations**, so it is safe to chain) reads the YAML directly — so it also works on directories not yet in `BOOK_DIRS` — and prints what the test can only tell you once a whole book exists: pooled correct-answer positions, uniquely-longest/shortest percentages, and the share of options carrying terminal punctuation, plus per-chapter hard violations, gutted or unterminated `explanationZh`, positional references, and a quiz↔check-in token-overlap heuristic. The punctuation figure is book-wide on purpose: the per-question check only asks that the four options in one question agree, so a pass that restyles a whole book stays green. `--review <chapter-id>` lays that chapter's `summaryZh` beside every stem and correct answer, which is the only practical way to run the no-spoiler check (summaries are Chinese, questions English — no machine can diff them).
