...
Email verification and password reset ([ADR 0027](adr/0027-transactional-email-resend.md)): a non-blank `RESEND_API_KEY` (`src/lib/email/` — Resend sender, bilingual templates, never-throwing `sendEmail`) turns on `requireEmailVerification` for password sign-up (sign-up then returns no session until the emailed link is clicked; unverified sign-in 403s and the login page re-sends the link) plus the forgot/reset-password flow (`revokeSessionsOnPasswordReset`). The public `(auth)` pages `/verify-email`, `/forgot-password`, `/reset-password` 404 when keyless. Account linking runs on better-auth's defaults — (IdP-verified email) AND (local `emailVerified`) — so verified password users link with Google/GitHub sign-ins automatically, while unverified accounts still fail `account_not_linked` (the anti-takeover property). `resetPassword` also creates a credential account for OAuth-only users, doubling as the "set a password" flow.
