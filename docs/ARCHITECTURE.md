...
└── lib/
    ...
    ├── book-core.ts      # book sanitization, quiz grading, chapter locks (pure)
    ...
    ├── membership-core.ts # tier resolution + per-tier entitlements (pure)
...
- **Content tables** (seeded, ids are stable slugs like `toeic-r-001`): `vocab_words`, `reading_exercises`, `listening_exercises`, `writing_prompts`, `speaking_prompts`, `mock_exams`, `books` / `book_chapters` (track-agnostic — CEFR level + genres instead of a track; `books.sort_order` is the difficulty-ladder position, seeded from `BOOK_DIRS` array order in `src/content/load.ts`; `origin`/`ownerUserId` keep the table tolerant of future user-uploaded books). Structured payloads (questions, scripts, checklists, exam sections, chapter sections) live in JSON columns typed via Drizzle's `$type<>()` against the zod-inferred types in `src/content/types.ts` — seed data and queries share one source of truth.
- **Progress tables** (per user): `srs_cards` (SM-2 state, unique per user+word), `review_logs`, `exercise_attempts`, `book_quiz_attempts` (chapter recall quizzes; completion and sequential unlock derive from these rows — there is no separate book-progress table), `writing_submissions` / `speaking_submissions` (with stored AI feedback JSON), `mock_exam_attempts` (deadlines, answers, scores, estimate), `activity_days` (streaks), `mistake_clears` (notebook re-test clears), `user_settings` (IANA timezone, reminder opt-in).
...
- **Membership** (`memberships`, one row per **premium** user — [ADR 0025](adr/0025-membership-roles-and-tiers.md)): absence of row = free tier, so there is no backfill and revoke is a DELETE. Server-owned (written only by admin actions; `source` is `'manual'` until a billing provider lands). `src/lib/membership-core.ts` resolves the row to a tier (`expiresAt` in the past = free) and maps tiers to entitlements — currently the chat and AI-grading daily limits, with premium env overrides clamped to never sit below the free values. Pre-billing demand is captured in `premium_interest` (one PK-idempotent row per user who requested access on `/premium`, cascade-deleted with the user; read by ops via SQL until billing exists).
...
Social sign-in ([ADR 0026](adr/0026-social-signin-google-github.md)): Google/GitHub OAuth registers only when a provider's `*_CLIENT_ID`/`*_CLIENT_SECRET` env vars are both set (`src/lib/auth/social-providers.ts` feeds both the server config and the login/register buttons — keyless environments show no trace of the feature).

Email verification and password reset ([ADR 0027](adr/0027-transactional-email-resend.md) + [ADR 0028](adr/0028-soft-email-verification.md)): a non-blank `RESEND_API_KEY` (`src/lib/email/` — Resend sender, bilingual templates, never-throwing `sendEmail`) sends a verification email on password sign-up and enables the forgot/reset-password flow (`revokeSessionsOnPasswordReset`). Verification is **soft**: sign-up always issues a session, sign-in never blocks, and unverified users see the persistent `VerifyEmailBanner` instead. The one verification-gated operation is explicit social linking — the auth route handler 403s `POST /api/auth/link-social` for unverified sessions, because better-auth's explicit link endpoint checks nothing about the local user's `emailVerified` (only implicit linking does), and a `hooks.before` gate can't see bearer sessions (the bearer plugin's header→cookie conversion is invisible to hooks). The public `(auth)` pages `/verify-email`, `/forgot-password`, `/reset-password` 404 when keyless. Implicit account linking runs on better-auth's defaults — (IdP-verified email) AND (local `emailVerified`) — so verified password users link with Google/GitHub sign-ins automatically, while unverified accounts still fail `account_not_linked` (the anti-takeover property). `resetPassword` also creates a credential account for OAuth-only users, doubling as the "set a password" flow.
...
Any completed learning action upserts one `activity_days` row per learner-local day in the same transaction as its primary progress row. The IANA timezone lives on `user_settings` (default `Asia/Shanghai`; [ADR 0014](adr/0014-learner-calendar-and-atomic-onboarding.md)); `calendarDay` and `computeStreak` are pure and DST-safe. `computeStreak` walks back from today — or yesterday, so a streak isn't shown broken before the first exercise of the day. Vocabulary uses SM-2 (see [ADR 0003](adr/0003-sm2-over-fsrs.md)) with four grade buttons; cards are created lazily on first review, and existing-card grades lock the row before scheduling so concurrent reviews cannot lose an advancement. The grade buttons show no interval previews — scheduler internals are deliberately hidden from learners (the tutor schedules for you); the v1 API still returns each card's `srs` state for clients that want it. The web study queue reviews due cards across all tracks and tops up new words goal-pack-first with spillover; the word list organizes by scenario (topic), not track ([ADR 0029](adr/0029-personal-collections-are-unfiltered.md)).
...
## Book reading (extensive reading + recall)

`/books` is track-agnostic extensive reading ([ADR 0024](adr/0024-book-reading-pregenerated-recall.md)): curated public-domain books read chapter by chapter, with retrieval practice built in — low-stakes **check-ins** at section breaks and a graded **end-of-chapter quiz** taken with the prose hidden. All questions are pre-generated at authoring time (`scripts/ingest-book.ts` + `scripts/generate-book-questions.ts`, human-reviewed before commit); **runtime never generates book questions**, so the recall surface costs nothing per reader and works keyless with no degradation state. The library lists in ladder order grouped by CEFR band, with a recommended-start hint mapped from the learner's self-rating (`book-core.recommendedBands`) — guidance only, never a cross-book gate.

Integrity mirrors exams: `book_chapters.sections`/`quiz` jsonb carry the keys, and every client read passes through `book-core.sanitizeChapter()`. Check-ins are stateless — answering asks the server for a reveal (`findCheckIn` matches check-ins only, so the endpoint cannot oracle quiz answers) and records nothing. The quiz submit is the persisted event (`book_quiz_attempts`, idempotent by requestId like exercise attempts): it feeds the mistakes notebook (kind `book_quiz`, `track NULL` — visible under every track filter), the `reading` skill signal, and the streak. Chapters hard-lock sequentially — chapter N+1 is unreadable until N's quiz is submitted (any score; completion-gated) — enforced in the reader query, both mutation services, and the API (409 `chapter_locked`). Completion/lock/current-chapter all derive from attempt rows; there is no progress table. List reads and the plan's continue-reading lookup use the seed-written `book_chapters.quiz_count` denorm (display/estimates only — grading always reads the quiz jsonb). The daily plan offers to continue the most-recently-active unfinished book (`book_chapter` item) but never starts one.

The **book tutor chat** ([ADR 0030](adr/0030-book-tutor-chat-premium-gated.md)) is the reader's one runtime AI surface and the first premium hard gate: `Entitlements.bookChatEnabled` flips by tier, and `src/lib/services/book-chat.ts` throws `PremiumRequiredError` (403 `premium_required`) for free users — after the keyless check, so a keyless server reports the feature absent (503) and the reader stays byte-identical for e2e. One rolling thread per (user, book) lives in `book_chat_messages`/`book_chat_states`, cloning the ADR 0013 machinery (learner-local day cap — per book, `MERCURY_BOOK_CHAT_DAILY_LIMIT` default 50 — sequence ordering, single-flight claim, atomic double-insert; failed calls persist nothing). Prompt context is whitelist-built in `src/lib/book-chat-core.buildBookChatContext`: prior chapters as title+summaryZh, the full current chapter's prose (the page shows it all anyway), never later chapters, and structurally never `correctIndex`/`explanationZh` — the input type has no slot for them. `bookTutorSystemPrompt` layers behavior on top: no plot beyond the current chapter, no resolving MCQ-shaped questions (coach from the passage instead), always encourage finishing the book. The UI is a docked corner panel plus a text-selection quote affordance, mounted in `BookReaderRunner`'s reading branch so quiz mode unmounts it.
