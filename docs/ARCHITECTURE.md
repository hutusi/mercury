...
The database is Postgres, addressed by `DATABASE_URL` — Neon's pooled connection in production, a local/containerized Postgres in dev/CI/e2e. The Drizzle handle wraps a lazily-connecting `pg.Pool` and is cached on `globalThis` because dev hot-reload re-evaluates modules and would otherwise leak pools. The pool is sized `max: 20` so a dashboard render's ~25-query fan-out runs largely in parallel (~1–2 round-trip waves instead of the ~5–6 a `max: 5` pool forced), and it's registered with `attachDatabasePool` so Vercel Fluid Compute cleans up its connections as an instance suspends (rather than leaking them across suspensions); Vercel functions are pinned to the Neon region (`vercel.json` `regions`) so those round-trips stay in-region ([ADR 0007](adr/0007-postgres-neon-for-serverless.md)).
