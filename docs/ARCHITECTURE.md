...
Listening exercises play pre-generated neural audio ([ADR 0021](adr/0021-pregenerated-listening-audio.md)): `bun run content:audio` renders each script to one MP3 via DashScope `qwen3-tts-flash` (distinct A/B/narrator voices) and uploads it to Vercel Blob ([ADR 0022](adr/0022-listening-audio-on-vercel-blob.md)) at the canonical pathname `audio/listening/<id>.<hash>.mp3`, tracked in `content/audio-manifest.json` (hash of model, voice cast, renderer settings, script). The seed links `listening_exercises.audio_url` (an origin-relative path) only when that hash still matches the seeded script, so stale audio can never play against an edited transcript — it degrades instead; the read layer prefixes `MERCURY_AUDIO_BASE_URL` to produce the fetchable Blob URL. Hash-immutable URLs upload with a year-long `cacheControlMaxAge`, so replays come from the browser cache. `public/audio/listening/` is only a gitignored local render cache. `TtsPlayer` plays the MP3 through a chrome-less `<audio>` element (no native controls: seeking would bypass the exam single-play rule) and falls back to browser TTS when `audio_url` is null or the media genuinely errors, mirroring the AI degradation philosophy.
