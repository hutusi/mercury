...
- **Auth is four layers**: `src/proxy.ts` (cookie check, UX only) → `(app)/layout.tsx` (real session) → **every server action calls `requireUser()`** (layouts don't protect actions) → **every `/api/v1` route calls `requireUserApi()`/`requireOnboardedApi()`** (401/403 envelopes, bearer tokens for native clients; the onboarding gate is `getOnboardedState()` — `settings.onboardedAt` + `learner_profiles.goalTrack`). Social sign-in (ADR 0026): Google/GitHub register only when both `*_CLIENT_ID`/`*_CLIENT_SECRET` env vars are set (`src/lib/auth/social-providers.ts` feeds server config + buttons; keyless = feature absent, which e2e relies on). Transactional email (ADR 0027): a non-blank `RESEND_API_KEY` (`src/lib/email/`) enforces email verification on password sign-up (**sign-up then returns `token: null`, no session — keyless must stay off or e2e/native contracts break**), enables password reset, and thereby unlocks better-auth's default password↔OAuth linking (requires IdP-verified AND local emailVerified; unverified accounts still get `account_not_linked` — intended). `/verify-email`, `/forgot-password`, `/reset-password` are public (auth) pages that 404 keyless; verification callbacks must land on `/verify-email`, never a protected path (the proxy's login bounce drops query strings).
