...
bun run content:audio      # re-render audio (listening/exam scripts + vocab headwords) → Vercel Blob (needs BLOB_READ_WRITE_TOKEN + DASHSCOPE_API_KEY; commit the manifest only)
...
- **Auth is four layers**: `src/proxy.ts` (cookie check, UX only) → `(app)/layout.tsx` (real session) → **every server action calls `requireUser()`** (layouts don't protect actions) → **every `/api/v1` route calls `requireUserApi()`/`requireOnboardedApi()`** (401/403 envelopes, bearer tokens for native clients; the onboarding gate is `getOnboardedState()` — `settings.onboardedAt` + `learner_profiles.goalTrack`). Social sign-in (ADR 0026): Google/GitHub register only when both `*_CLIENT_ID`/`*_CLIENT_SECRET` env vars are set (`src/lib/auth/social-providers.ts` feeds server config + buttons; keyless = feature absent, which e2e relies on). Transactional email (ADR 0027) + soft verification (ADR 0028): a non-blank `RESEND_API_KEY` (`src/lib/email/`) sends a verification email on password sign-up and enables password reset — but **never blocks sign-in/sign-up** (every env issues a session immediately; e2e and the native contract depend on it). Unverified users get the persistent `VerifyEmailBanner` in the (app) layout (gated `isEmailEnabled && !emailVerified` — keyless e2e users are all unverified), and the ONE verification-gated operation is explicit social linking: the auth route handler 403s `POST /api/auth/link-social` for unverified sessions (better-auth's explicit link endpoint checks nothing about local emailVerified, and a `hooks.before` gate can't see bearer sessions — the bearer header→cookie conversion is invisible to hooks; implicit linking's default gate stays, so unverified accounts still get `account_not_linked` on OAuth sign-in, intended). `/verify-email`, `/forgot-password`, `/reset-password` are public (auth) pages that 404 keyless; verification callbacks must land on `/verify-email`, never a protected path (the proxy's login bounce drops query strings).
...
- Book reading (ADR 0024): track-agnostic extensive reading under `/books` — per-chapter YAML in `content/books/`, questions pre-generated offline and human-reviewed (**runtime never generates book questions**). Check-ins are a stateless server reveal; the end-of-chapter quiz is the persisted event (`book_quiz_attempts` — completion + sequential hard lock derive from it, no progress table). Quiz misses land in mistakes as `book_quiz` with `track NULL`; the plan's `book_chapter` item continues a started book, never starts one. Sanitization via `src/lib/book-core.ts`. The **book tutor chat** (ADR 0030) is the reader's one runtime AI surface: premium-only (the first hard gate — `Entitlements.bookChatEnabled` + `PremiumRequiredError` → 403 `premium_required`), one thread per (user, book) on `book_chat_messages`/`book_chat_states` (ADR 0013 machinery), capped per book per day (`MERCURY_BOOK_CHAT_DAILY_LIMIT`, default 50), absent keyless (enabled-before-entitled gate order — e2e relies on the unchanged reader). Prompt context is whitelist-built in `src/lib/book-chat-core.ts`: prior chapters as title+summaryZh, full current-chapter prose, never later chapters or answer keys.
...
- Learner model (ADR 0012): `learner_profiles` holds client-writable goals + **server-owned** `skillEstimates`/`coachMemo`, evolved only via `src/lib/learner-model-core.ts` (EWMA signals hooked into services with `recordSkillSignalSafely` — guarded, never fails the parent mutation; memo updates ride the grading call's optional `memoUpdate`). The dashboard 今日计划 comes from `src/lib/plan-core.ts` — deterministic rules, deliberately not an AI call. Tutor chat (ADR 0013): single rolling thread, non-streaming, per-user daily cap → 429 `chat_limit_reached`; the cap is membership-tier-aware (ADR 0025): `MERCURY_CHAT_DAILY_LIMIT` (default 30) for free, `MERCURY_CHAT_DAILY_LIMIT_PREMIUM` (default 100) for premium, resolved via `src/lib/membership-core.ts`.
...
- Speech APIs (TTS/STT) are client-only: components gate on a `mounted` effect state to avoid hydration mismatches; TTS speaks one utterance per script line and must cancel on unmount. Listening/exam scripts and vocab headwords prefer pre-generated MP3s on Vercel Blob (ADR 0021/0022/0023: `content:audio` uploads by content hash, commits only `content/audio-manifest.json`; seed links relative paths on a fresh hash — exam URLs ride the sections jsonb into attempt snapshots; reads prefix `MERCURY_AUDIO_BASE_URL` via `src/lib/audio-url.ts`) with browser TTS as the fallback — after editing scripts or headwords, regenerate or the surface silently degrades. Vocab example sentences deliberately stay on browser TTS. `public/audio/` is a gitignored local render cache, never committed.
