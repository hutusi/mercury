...
- Book reading (ADR 0024): track-agnostic extensive reading under `/books` — per-chapter YAML in `content/books/`, questions pre-generated offline and human-reviewed (**runtime never calls AI for books**). Check-ins are a stateless server reveal; the end-of-chapter quiz is the persisted event (`book_quiz_attempts` — completion + sequential hard lock derive from it, no progress table). Quiz misses land in mistakes as `book_quiz` with `track NULL`; the plan's `book_chapter` item continues a started book, never starts one. Sanitization via `src/lib/book-core.ts`.
