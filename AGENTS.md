...
bun run content:book-questions # legacy AI drafter for book check-ins/quizzes — scaffolding is now hand-authored (see docs/CONTENT.md)
bun run content:book-audit     # advisory book-scaffolding stats: pooled answer-position spread, length skew, per-chapter violations
```
...
- Book reading (ADR 0024): track-agnostic extensive reading under `/books` — per-chapter YAML in `content/books/`, questions authored offline by hand (**runtime never calls AI for books**). Check-ins are a stateless server reveal; the end-of-chapter quiz is the persisted event (`book_quiz_attempts` — completion + sequential hard lock derive from it, no progress table). Quiz misses land in mistakes as `book_quiz` with `track NULL`; the plan's `book_chapter` item continues a started book, never starts one. Sanitization via `src/lib/book-core.ts`.
