...
- **Auth is four layers**: `src/proxy.ts` (cookie check, UX only) → `(app)/layout.tsx` (real session) → **every server action calls `requireUser()`** (layouts don't protect actions) → **every `/api/v1` route calls `requireUserApi()`/`requireOnboardedApi()`** (401/403 envelopes, bearer tokens for native clients; the onboarding gate is `learner_profiles.goalTrack` presence).
