...
bun run content:audio      # re-render listening MP3s after editing scripts (needs DASHSCOPE_API_KEY; commit MP3s + manifest)
```
...
- Speech APIs (TTS/STT) are client-only: components gate on a `mounted` effect state to avoid hydration mismatches; TTS speaks one utterance per script line and must cancel on unmount. Listening exercises prefer pre-generated MP3s (ADR 0021: `content:audio` → `public/audio/listening/` + manifest, seed links `audio_url` only on a fresh content hash) with browser TTS as the fallback — after editing a listening script, regenerate or the exercise silently degrades.
...
- [docs/adr/](docs/adr/) — decision records (Bun+Node split, SQLite+Drizzle, SM-2, homegrown i18n, server-issued deadlines, AI degradation, Postgres/Neon for serverless, versioned migrations, YAML content authoring, HTTP API v1 + bearer auth, multi-provider AI, learner model + AI memory, tutor chat, track as goal + content filter)
