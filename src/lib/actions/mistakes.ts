...
export async function retestMistake(input: z.infer<typeof RetestSchema>): Promise<RetestResult> {
  ...
  if (correct) {
    await upsertClear(user.id, kind, refId, questionId);
  }
...
const VocabRetestSchema = z.object({
  wordId: z.string(),
  chosenWordId: z.string(),
});

export async function retestVocabMistake(
  input: z.infer<typeof VocabRetestSchema>,
  ...
  if (!word) throw new Error(`Unknown vocab word: ${wordId}`);
  ...
  if (correct) {
    ...
    await upsertClear(user.id, "vocab_quiz", `quiz-${word.track}`, wordId);
  }
  await recordActivity(user.id);
