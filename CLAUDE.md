...
- **Auth is four layers**: `src/proxy.ts` (cookie check, UX only) → `(app)/layout.tsx` (real session) → **every server action calls `requireUser()`** (layouts don't protect actions) → **every `/api/v1` route calls `requireUserApi()`/`requireTrackApi()`** (401/403 envelopes, bearer tokens for native clients).
...
- [docs/adr/](docs/adr/) — decision records (Bun+Node split, SQLite+Drizzle, SM-2, homegrown i18n, server-issued deadlines, AI degradation, Postgres/Neon for serverless, versioned migrations, YAML content authoring, HTTP API v1 + bearer auth, multi-provider AI)
